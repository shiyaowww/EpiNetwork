...
for (i in 1:length(probLowSes))
{
  ...
  for (j in 1:length(nodeMatch))
  {
    ...
    for (k in 1:length(suscScale))
    {
      ...
      # sum this scenario
      dfSumScenario <- data.frame(steps = timeRun/stepLength,
      ...
      # calculate and store the mean (and sd) of endemic prevalence over last 20 steps 
      tableAvgEP[j,k] <- 
        avgEndemicPrev <- mean(dfSumScenario$prev[(timeRun/stepLength-19):timeRun/stepLength])
      tableSdEP[j,k] <-
        sdEndemicPrev <- sd(dfSumScenario$prev[(timeRun/stepLength-19):timeRun/stepLength])
      
      # calculate and store the mean (and sd) of incidence ratio over all steps  
      tableAvgIR[j,k] <-
        avgIncidenceRatio <- mean(dfSumScenario$incidenceRatio[1:timeRun/stepLength])
      tableSdIR[j,k] <- 
      ...
      tableAvgPR[j,k] <-
        avgPrevRatio <- mean(dfSumScenario$prevRatio[1:timeRun/stepLength])
      tableSdPR[j,k] <-
...
# pdf("/Users/shiyaow/Desktop/0131_contour.pdf")
